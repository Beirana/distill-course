...
下载等待与 smoke 关键节点由助教主动提供一个实际文件/视频入口，不等人工提问；用户可选一起看、深入或继续，不要求答对。引导时给用户回应机会，代执行时简短说明并如实记为 Agent 讲解，不把自问自答视为用户已经理解。

## 视频与节奏

若课程材料可用，优先提供 `07_样本到LoRA_180秒_Edge晓晓配音.mp4`（3 分钟）的观看入口，用户选择后结合相应文件。可补 `03_蒸馏总览_62秒_Edge晓晓配音.mp4`、`05_LoRA_二维公式讲解_v3.mp4`，以及 `06_第二三次视频之间的smoke讲解.mp4` 的相关片段。视频不随独立 Skill 分发：先发现现有路径；缺失时直接讲文件，不要求重新下载视频，也不臆造时间戳。找到文件不等于已经播放或用户已看完。
...
前两条二选一：每秒刷新 GPU，`Ctrl+C` 仅停止监视。`df` 看文件系统余量，路径按实际盘替换。`tail -n` 看尾部，`tail -f` 追踪追加；退出 tail 不会停止训练。`json.tool` 展示 JSON；`ss` 看监听端口。下载进度看实际下载器的日志和完成校验，只有选用 aria2 才看 aria2 日志，不看预分配后的目录大小；不要把包含签名 URL 的原始下载日志贴进公开文档。
